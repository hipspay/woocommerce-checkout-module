/*global jQuery, wchipsPaymentRequestParams, PaymentRequest, hips, Promise */
/*jshint es3: false */
/*jshint devel: true */
(function( $ ) {

	/**
	 * WooCommerce hips PaymentRequest class.
	 *
	 * @type {Object}
	 */
	var wchipsPaymentRequest = {
		...
		getAjaxURL: function( endpoint ) {
			return wchipsPaymentRequestParams.ajax_url
				.toString()
				.replace( '%%endpoint%%', 'wc_hips_' + endpoint );
		},
		...
		initPaymentRequest: function( evt ) {
			evt.preventDefault();
			var self = wchipsPaymentRequest;
			var data = {
				security: wchipsPaymentRequestParams.nonce.payment
			};
		...
		updateShippingOptions: function( details, address, resolve, reject ) {
			var self = this;
			var data = {
				security:  wchipsPaymentRequestParams.nonce.shipping,
		...
		updateShippingDetails: function( details, shippingOption, resolve, reject ) {
			...
			var selected = null;
			var data     = {
				security:  wchipsPaymentRequestParams.nonce.update_shipping,
				shipping_method: [
					shippingOption
				]
			};
			...
			$.ajax({
				...
				success: function( response ) {
					...
					if ( null === selected ) {
						reject( wchipsPaymentRequestParams.i18n.unknown_shipping.toString().replace( '[option]', shippingOption ) );
					}
		...
		getOrderData: function( payment ) {
			...
			var data     = {
				_wpnonce:                  wchipsPaymentRequestParams.nonce.checkout,
		...
		processPayment: function( payment ) {
			...
			hips.setPublishableKey( wchipsPaymentRequestParams.hips.key );
			hips.createToken( cardData, function( status, response ) {
				if ( response.error ) {
					self.abortPayment( payment, self.getErrorMessageHTML( response.error.message ) );
				} else {
					// Check if we allow prepaid cards.
					if ( 'no' === wchipsPaymentRequestParams.hips.allow_prepaid_card && 'prepaid' === response.card.funding ) {
						self.abortPayment( payment, self.getErrorMessageHTML( wchipsPaymentRequestParams.i18n.no_prepaid_card ) );
					} else {
						// Token contains id, last4, and card type.
						orderData.hips_token = response.id;
